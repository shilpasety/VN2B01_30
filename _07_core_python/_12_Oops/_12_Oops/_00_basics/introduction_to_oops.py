...
class Person:
  def __init__(emp, name, age, id): # attributes
    emp.name = name  # instance variables- self.name, self.age
    emp.age = age    # local variables - name, age
    emp.id = id
...
approach and used where computation is done by using objects.	1.Procedural programming uses a list of instructions
                                                                to do computation step by step.
2.	It makes the development and maintenance easier.	        2.In procedural programming, It is not easy to maintain
                                                                the codes when the project becomes lengthy.
3.	It simulates the real world entity.
So real-world problems can be easily solved through oops.	    3.It doesn't simulate the real world. It works on step by step instructions divided into small parts called functions.
4.	It provides data hiding. So it is more secure
than procedural languages.                                      4.You cannot access private data from anywhere.
...
is C++, Java, .Net, Python, C#, etc.	                        5.Example of procedural languages are: C, Fortran, Pascal, VB etc.
